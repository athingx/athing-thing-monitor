package io.github.athingx.athing.aliyun.monitor.impl.usage;
...
import static io.github.athingx.athing.aliyun.monitor.impl.util.MathUtils.percent;
import static io.github.athingx.athing.aliyun.monitor.impl.util.MathUtils.scale;

public class UsageThingComImpl implements UsageThingCom {
    ...
    private float[] calculateCpuUsage(CentralProcessor processor) {
        ...
        Arrays.stream(TickType.values())
                .mapToInt(TickType::getIndex)
                .forEach(index -> dRates[index] = percent(dTicks[index], total));
    ...
    public Usage.Memory getMemory() {
        ...
        return new Usage.Memory(
                memory.getAvailable(),
                percent(memory.getAvailable(), memory.getTotal()),
                memory.getVirtualMemory().getSwapUsed(),
                percent(memory.getVirtualMemory().getSwapUsed(), memory.getVirtualMemory().getSwapTotal())
        );
    }
    ...
    public Usage.Power[] getPowers() {
        return hardware.getPowerSources().stream()
                .map(power ->
                        new Usage.Power(
                                power.getName(),
                                scale(power.getRemainingCapacityPercent())
                        )
                )
        ...
        return os.getFileSystem().getFileStores().stream()
                .map(store ->
                        new Usage.Store(
                                ...
                                percent(store.getUsableSpace(), store.getTotalSpace())
                        )
                )
